...
//volatile extern UINT32 tValue;
 
/* Interprocess Communications*/
int send_message (int process_ID, void * MessageEnvelope){
    // For purposes of timing analysis
    // Reset timer value
    tValue = 0;
    // Enable microsecond timer to measure kernel time
    TIMER0_TMR = 0x001B;
...
int delayed_send(int process_ID, void * MessageEnvelope, int delay){
	...
	//save d0 and place the address of the process_ID to be replaced into d0
	asm( "move.l %0, %%d1" : : "g" (process_ID) );
	//save d1 and place the address of the MessageEnvelope to be replaced into d1
	asm( "move.l %0, %%d2" : : "g" (MessageEnvelope) );
	//save d2 and place the address of the delay to be replaced into d2
	asm( "move.l %0, %%d3" : : "g" (delay) );
...
int set_process_priority (int process_ID, int priority){
	...
	//save d1 and place the address of the process_ID to be replaced into d1
	asm( "move.l %0, %%d1" : : "g" (process_ID) );
	//save priority into d2 for kernel to read
	asm( "move.l %0, %%d2" : : "g" (priority) );
	
	//call trap to go to kernel level primitive
	asm( "trap #0" );
	
	//Check the returnCode stored in D0 to see if priority was set properly
	asm( "move.l %%d0, %0" : "=g" (returnCode) : );
...
int get_process_priority (int process_ID){
	...
	//Check the returnCode stored in D0 to see the return priority (-1 on error)
	asm( "move.l %%d0, %0" : "=g" (returnCode) : );
