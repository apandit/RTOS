...
int main() 
{
	...
	ProcA = CreateProcess( pcbs, &ProcessA, AllocateStack(4096), m_nextPid++, MEDIUM ); 
	ProcB = CreateProcess( pcbs, &ProcessB, AllocateStack(4096), m_nextPid++, LOW ); 
	ProcC = CreateProcess( pcbs, &ProcessC, AllocateStack(4096), m_nextPid++, LOW ); 	
    ...
    asm( "move.l #timerAnalysisEntry,%d0" );
    ...
    //Set the reference counts, ~100us
    TIMER0_TRR = 450;
