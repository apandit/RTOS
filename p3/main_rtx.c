...
int main() 
{
	...
	rtxProcess* cp1 = CreateProcess( pcbs, &keyboardCommandDecoder, AllocateStack(2048), KCDPID, MEDIUM );
	rtxProcess* cp2 = CreateProcess( pcbs, &CRTDisplay, AllocateStack(1024), CRTPID, MEDIUM);
	rtxProcess* cp3 = CreateProcess( pcbs, &UART_PROCESS, AllocateStack(1024), UARTPID, HIGH );
	rtxProcess* cp4 = CreateProcess( pcbs, &TIMER_PROCESS, AllocateStack(1024), TIMERPID, HIGH );
	rtxProcess* cp5 = CreateProcess( pcbs, &WallClock, AllocateStack(1024), WALLPID, HIGH );
	...
	EnqueueProcess( rtxProcMan, cp5,READYQUEUE );


    if( cp1 == NULL || cp2 == NULL || cp5 == NULL /*|| cp6 == NULL */){
	...
	rtx_dbug_outs( (CHAR*)"All free memory: " );
	WriteLine();
	
	MemoryBlock* tmp = freeMemory.head;
	while( 0 ){
		WriteHex( tmp->hack );
		WriteLine();
		tmp = tmp->next;
